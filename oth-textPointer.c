...
void computer_decision(int mesh[NUM_OF_ELEMENTS][NUM_OF_ELEMENTS], int point[NUM_OF_ELEMENTS][NUM_OF_ELEMENTS]);
int boundary_display(int, int, int); /* 　境界空白の表示　*/
int boundary_element(int, int mesh[NUM_OF_ELEMENTS][NUM_OF_ELEMENTS]); /* 境界空白の論理的要素 */
...
int main(void)
{
    ...
    int point[NUM_OF_ELEMENTS][NUM_OF_ELEMENTS];
    int win; /* 描画用のWindow */
...
//    int i, j;
    ...
    while(availables > 0) {
        ...
        if(stone == BLACK_STONE){
	     ...
	     printf("次は白です．");
         printf("どこに置きますか? ");
         computer_decision(mesh, point);
            do{
            
                if(mesh[row][column]!=BOUNDARY_BIT){
                    row = rand() % 8;
                    column = rand () % 8;
                }
                else
...
void computer_decision(int mesh[NUM_OF_ELEMENTS][NUM_OF_ELEMENTS],
                      int point[NUM_OF_ELEMENTS][NUM_OF_ELEMENTS])
{
    int i, j;
...
int boundary_element(int win, int mesh[NUM_OF_ELEMENTS][NUM_OF_ELEMENTS]){
    int i, j;
    for(i = 0; i < NUM_OF_ELEMENTS; i++){
        for(j = 0; j < NUM_OF_ELEMENTS; j++){
            if(mesh[i][j] == BOUNDARY_BIT)
                boundary_display(win, i, j);
        }
    }
    return 0;
}
	    ...
	    int stone) /* この色の石で (row, column) にある石を挟む */
{
  ...
  int adjacent_row = 0;
  int adjacent_column = 0; /* (row, column)のさらに隣 */
	     ...
	     int way)
{
  int *adjacent;  /* (row, column)の隣の要素へのポインタ */
  int adjacent_row = 0;
  int adjacent_column = 0; /* (row, column)の隣 */
